...
namespace XiaoCai.WinformUI.Docking
{
	...
	public class DockContentHandler : IDisposable, IDockDragSource
	{
		...
		public double AutoHidePortion
		{
			...
			{
				if (value <= 0)
                    throw (new ArgumentOutOfRangeException(XiaoCai.WinformUI.Properties.Resources.DockContentHandler_AutoHidePortion_OutOfRange));
		...
		public DockAreas DockAreas
		{
			...
			set
			{
				...
				if (!DockHelper.IsDockStateValid(DockState, value))
                    throw (new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockContentHandler_DockAreas_InvalidValue));
		...
		public bool IsFloat
		{
			...
			set
			{
				...
				if (visibleState == DockState.Unknown)
                    throw new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockContentHandler_IsFloat_InvalidValue);
			...
			set
			{
				...
				if (value != null)
				{
					if (value.IsFloat || value.DockPanel != DockPanel)
                        throw new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockContentHandler_DockPane_InvalidValue);
				}
		...
		public DockPane FloatPane
		{
			...
			set
			{
				...
				{
					if (!value.IsFloat || value.DockPanel != DockPanel)
                        throw new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockContentHandler_FloatPane_InvalidValue);
				}
		...
		public DockState ShowHint
		{
			...
			{	
				if (!DockHelper.IsDockStateValid(value, DockAreas))
                    throw (new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockContentHandler_ShowHint_InvalidValue));
		...
		{
			if (dockPanel == null)
                throw (new ArgumentNullException(XiaoCai.WinformUI.Properties.Resources.DockContentHandler_Show_NullDockPanel));
		...
		public void Show(DockPanel dockPanel, DockState dockState)
		{
			...
			if (dockState == DockState.Unknown || dockState == DockState.Hidden)
                throw (new ArgumentException(XiaoCai.WinformUI.Properties.Resources.DockContentHandler_Show_InvalidDockState));
