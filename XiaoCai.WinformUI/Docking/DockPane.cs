	...
	public partial class DockPane : UserControl, IDockDragSource
	{
			...
			set
			{
				...
				if (value != null)
				{
					if (!DisplayingContents.Contains(value))
                        throw (new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockPane_ActiveContent_InvalidValue));
				}
				else
				{
		...
		protected override void OnLayout(LayoutEventArgs levent)
		{
			...
			if (!IsHidden)
			{
			    ...
			    //TabStripControl.BackColor = Color.Red;
		...
		public void SetContentIndex(IDockContent content, int index)
		{
			...
			if (oldIndex == -1)
                throw (new ArgumentException(XiaoCai.WinformUI.Properties.Resources.DockPane_SetContentIndex_InvalidContent));

			if (index < 0 || index > Contents.Count - 1)
				if (index != -1)
                    throw (new ArgumentOutOfRangeException(XiaoCai.WinformUI.Properties.Resources.DockPane_SetContentIndex_InvalidIndex));
		...
		public DockPane SetDockState(DockState value)
		{
			if (value == DockState.Unknown || value == DockState.Hidden)
                throw new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockPane_SetDockState_InvalidState);
		...
		{
			if (container == null)
                throw new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockPane_DockTo_NullContainer);
		...
		private void InternalAddToDockList(INestedPanesContainer container, DockPane prevPane, DockAlignment alignment, double proportion)
		{
			if ((container.DockState == DockState.Float) != IsFloat)
                throw new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockPane_DockTo_InvalidContainer);

			int count = container.NestedPanes.Count;
			if (container.NestedPanes.Contains(this))
				count --;
			if (prevPane == null && count > 0)
                throw new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockPane_DockTo_NullPrevPane);

			if (prevPane != null && !container.NestedPanes.Contains(prevPane))
                throw new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockPane_DockTo_NoPrevPane);

			if (prevPane == this)
                throw new InvalidOperationException(XiaoCai.WinformUI.Properties.Resources.DockPane_DockTo_SelfPrevPane);
        ...
        {
            if (panel != DockPanel)
                throw new ArgumentException(XiaoCai.WinformUI.Properties.Resources.IDockDragSource_DockTo_InvalidPanel, "panel");
